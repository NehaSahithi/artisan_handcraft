...
export default function SellerDashboard() {
  ...
  const fetchStudioData = async () => {
    ...
    try {
      ...
      const statsRes = await apiClient.get('/api/orders/sales-stats')
      if (statsRes.data?.success) {
        setStats(statsRes.data.stats || { totalOrders: 0, totalRevenue: 0, avgOrderValue: 0 })
      }
                ...
                <tbody className="divide-y divide-border/50">
                  <AnimatePresence>
                    {recentOrders.map((order) => {
                      // Filter items specifically belonging to this artisan
                      const myItems = order.items.filter(item => 
                        item.artisan?._id === user._id || item.artisan === user._id
                      )
                      
                      // Skip rendering if no items belong to this artisan in a multi-artisan order
                      if (myItems.length === 0) return null;

                      // Derive status from the first item (assuming batch updates for simplicity here)
